...
-- Class(base, ...): FS25's OO helper. Returns a metatable whose __index chains to
-- base, which is enough for `setmetatable({}, Class(Foo))` and method dispatch in
-- tests. Extra args (e.g. Class(Foo, Event)) are ignored; the first arg wins.
function Class(base)
  local mt = {}
...
-- ordinary log noise (MDMLog.print, etc.) is ignored.
T = { _pass = 0, _fail = 0 }
