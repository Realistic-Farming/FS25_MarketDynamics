<#
.SYNOPSIS
    Extracts MarketDynamics-related errors and warnings from FS25 game log.

.DESCRIPTION
    Parses the Farming Simulator 25 log.txt file and extracts all entries
    related to the MarketDynamics mod, categorizing them by severity and type.
...
Write-Header "MarketDynamics Log Analyzer"
...
# Patterns to match
$patterns = @{
    # MarketDynamics specific patterns
    MarketDynamicsLine = 'MDM|FS25_MarketDynamics'
    ModLoaded = 'v(\d+\.\d+\.\d+)'
    ...
    # Specific MarketDynamics patterns
...
foreach ($i in 0..($lines.Count - 1)) {
    ...
    # Check if line is MarketDynamics related
    if ($line -match $patterns.MarketDynamicsLine) {
        $stats.MarketDynamicsLines++
    ...
    if ($line -match $patterns.XmlParse -and $line -match 'MarketDynamics') {
        $stats.XmlErrors += @{
            Line = $i + 1
            Text = $line.Trim()
            Type = "XmlError"
...
$report = @()
$report += "=" * 60
$report += "MarketDynamics Error Report"
...
$report += "MarketDynamics-Related Lines: $($stats.MarketDynamicsLines)"
