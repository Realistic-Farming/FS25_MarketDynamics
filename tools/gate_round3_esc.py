#!/usr/bin/env python3
"""Round 3 Esc chrome assertion gate (2026-08-05).

Asserts the four round-3 edits from ENGINE-George-Esc-round3-2026-08-05:
  E1 nest glass hidden (visible=false, element + arrow kept)
  E2 rfHostTitle/rfHostBlurb gated off on CS (Lua) + 2b skipHostDuplex includes CS
  E3 RF_WcSideInfoShell 0px -80px (card top-aligned under selector cluster)
  E4 RF_MdGraphArea 30px -38px (10px air under graph title)
plus the round-2 keep list (pivot pane, underlay, table geometry, WC pages, Soil pH).
...
# ---- E1: nest glass hidden, element + arrow kept ----
host = find_by_id(gtree, "rfContentHost")
check("E1 rfContentHost exists", host is not None)
...
check("E1 nest ThreePartBitmap still second child",
      nest is not None and nest.tag == "ThreePartBitmap"
      and nest.get("profile") == "RF_SubCategoryContainerBg")
check("E1 nest visible=false", nest is not None and nest.get("visible") == "false")
arrow_kids = [c for c in (list(nest) if nest is not None else [])
              if c.get("profile") == "fs25_subCategoryContainerArrow"]
check("E1 arrow child kept under nest, visible=false",
      len(arrow_kids) == 1 and arrow_kids[0].get("visible") == "false")

# ---- E2: CS host title/blurb gate (Lua text assertions) ----
check("E2 isCs local defined in _syncHostGuestChrome",
      'local isCs = activeId == "seasonalCropStress"' in lua_src)
check("E2 rfHostTitle gated with not isCs",
      "setVis(self.rfHostTitle, not isWc and not isMd and not isCs)" in lua_src)
check("E2 rfHostBlurb gated with not isCs",
      "setVis(self.rfHostBlurb, not isWc and not isMd and not isCs)" in lua_src)
check("E2 old rfHostTitle gate (without isCs) gone",
      "setVis(self.rfHostTitle, not isWc and not isMd)\n" not in lua_src)
check("E2 old rfHostBlurb gate (without isCs) gone",
      "setVis(self.rfHostBlurb, not isWc and not isMd)\n" not in lua_src)
check("E2b skipHostDuplex includes seasonalCropStress",
...
# ---- E3: card top-aligned under selector cluster ----
wcshell = profs.get("RF_WcSideInfoShell")
check("E3 RF_WcSideInfoShell position 0px -80px", pval(wcshell, "position") == "0px -80px")
check("E3 RF_WcSideInfoShell size 380px 240px held", pval(wcshell, "size") == "380px 240px")
mdcard = find_by_id(gtree, "mdSideInfoShell")
check("E3 mdSideInfoShell shares RF_WcSideInfoShell profile (no inline position)",
...
check("E3 wcSideInfoShell shares profile (no inline position)",
...
check("E3 Soil/CS RF_SideInfoShell untouched at 8px -148px",
      pval(sideinfo, "position") == "8px -148px")

# ---- E4: graph air ----
graph = profs.get("RF_MdGraphArea")
check("E4 RF_MdGraphArea position 30px -38px", pval(graph, "position") == "30px -38px")
check("E4 RF_MdGraphArea size 720px 200px held", pval(graph, "size") == "720px 200px")
hint = find_by_id(gtree, "mdGraphEmptyHint")
check("E4 mdGraphEmptyHint held at 30px -96px",
...
check("KEEP RF_SideColUnderlay extends RF_SideColBg / width 404px",
      under is not None and under.get("extends") == "RF_SideColBg"
      and pval(under, "width") == "404px")
sidebg = profs.get("RF_SideColBg")
check("KEEP RF_SideColBg blank.png swatch 0.165 0.176 0.157 0.92",
